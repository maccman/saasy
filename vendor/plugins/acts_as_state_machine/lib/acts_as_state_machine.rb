    ...
    module StateMachine              #:nodoc:
      ...
      module SupportingClasses
        ...
        class StateTransition
          ...
          def perform(record)
            ...
            ## We're changing the behaviour of AASM here - when
            ## state_name! is called, all the attributes are saved,
            ## not just the state column.
      ...
      module ActMacro
        ...
        def acts_as_state_machine(opts)
          ...
          # My custom method to make use
          # of Rails' juicy new syntax
          named_scope :in_state, lambda {|*states|
            {:conditions => {state_column => states.collect(&:to_s)}}
          }
      ...
      module InstanceMethods
        # Edited so that it doesn't override any states
        # I specify before saving.
        ...
        def run_initial_state_actions
          initial = self.class.read_inheritable_attribute(:states)[current_state]
          initial.entering(self)
          initial.entered(self)
        end
      
        # Returns the current state the object is in, as a Ruby symbol.
        # Edited so that it doesn't call nil.to_sym
        def current_state
          (cs = self.send(self.class.state_column)) && cs.to_sym
        end
